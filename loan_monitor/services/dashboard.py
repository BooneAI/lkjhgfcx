...
async def gather_dashboard_snapshot(
    ...
    *,
    ...
    cur = conn.cursor()
    cur.execute("SELECT principal, interest, updated_at FROM loan WHERE id = 1")
    loan_row = cur.fetchone()
    if loan_row:
        principal, interest, loan_updated = loan_row
    else:
        principal = float(config.loan.get("principal", 0.0) or 0.0)
        interest = float(config.loan.get("interest", 0.0) or 0.0)
        loan_updated = None

    cur.execute(
        "SELECT btc_amount, usdt_amount, btc_price, created_at FROM collateral_snapshot "
    ...
    if collateral_row:
        ...
        btc_amount = float(config.collateral.get("btc", 0.0) or 0.0)
        usdt_amount = float(config.collateral.get("usdt", 0.0) or 0.0)
    ...
    thresholds = {
        ...
        "margin_call": config.thresholds.margin_call,
        "liquidation": config.thresholds.liquidation,
    }
